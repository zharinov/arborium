...
module.exports = grammar({
  ...
  externals: $ => [
    ...
    $.float_literal,
    $._outer_block_doc_comment_start,
    $._outer_line_doc_comment_start,
  ...
  conflicts: $ => [
    ...
    [$.trait_bounds], // due to allowing empty bounds
  ],
  ...
  rules: {
    ...
    macro_definition_v2: $ => seq(
      ...
      choice(
        seq(
          field('arguments', $.macro_arguments_declaration),
          field('body', $.macro_body_v2)
        ),
        field('rules', $.macro_rules_v2)
      )
    ),

    macro_arguments_declaration: $ => seq(
      '(',
    ...
    // this will accept '()', '[]' and '{}' for delimiters on the left and right,
    // whereas in v2 only '()' is legal on the left-hand side and '{}' on the right-hand side.
    // TODO: this could be made more restrictive
    macro_rule_v2: $ => seq(
    ...
    primitive_type: $ => choice(...primitiveTypes),
    ...
    _non_special_token: $ => choice(
      ...
      'if', 'impl', 'let', 'loop', 'match', 'mod', 'pub', 'return', 'raw', 'static', 'struct', 'trait',
    ...
    where_predicate: $ => seq(
      field('left', choice(
        $.lifetime,
        // TODO: should we just include `_type` here instead of listing all those variants?
        // quite some parse failures come from some particular kind of type needing to be added here…
        $._type_identifier,
    ...
    macro_invocation_item: $ => seq(
      optional(field('attributes', $.attributes)),
      field('invocation', $.macro_invocation)
    ),
    ...
    field_initializer: $ => seq(
      optional($.attributes),
      field('field', $._field_identifier_or_index),
      ':',
      field('value', $._expression),
    ),
    ...
    struct_pattern: $ => seq(
      ...
      '{',
      sepBy(',', choice(
        seq(optional($.attributes), $.field_pattern),
        $.remaining_field_pattern)
      ),
      optional(','),
      '}',
    ...
    field_pattern: $ => seq(
      ...
      choice(
        ...
        seq(
          field('name', $._field_identifier_or_index),
          ':',
          field('pattern', $._pattern),
        ),
      ),
    ...
    line_comment: $ => seq(
      // All line comments start with either two slashes or four slashes or more.
      // The case of three slashes (outer doc comments) is handled as a special case
      // by the lexer, which then emits the _outer_line_doc_comment_start token.
      '//',
      choice(
    ...
    line_outer_doc_comment: $ => seq(
      $._outer_line_doc_comment_start,
      field('doc', alias($._line_doc_content, $.doc_comment)),
    ),

    // Documentation block comments: /** docs */
    block_outer_doc_comment: $ => seq(
      $._outer_block_doc_comment_start,
    ...
    block_comment: $ => seq(
      ...
      optional(
        choice(
          // Inner documentation block comments: /*! docs */
          seq(
    ...
    _reserved_identifier: $ => alias(choice(
      // 'async', // TODO: this should be enabled but it creates a conflict with trait bounds
      'auto',
      'await',
      'default',
      // 'dyn', // TODO: this should be enabled but it creates a conflict with scoped type identifiers
      'gen',
      'macro_rules',
      'raw',
      'safe',
    ...
    _field_identifier_or_index: $ => choice($._field_identifier, $.integer_literal),
    ...
    metavariable: _ => seq(
      /\$\s*[_\p{XID_Start}][_\p{XID_Continue}]*/
    ),
  },
});
